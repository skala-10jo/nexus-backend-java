...
public interface ExpressionRepository extends JpaRepository<Expression, UUID> {
    ...
    /**
     * Unit별 표현 목록 조회 (개수 제한)
     */
    @Query(value = "SELECT * FROM expressions WHERE unit = :unit ORDER BY RANDOM() LIMIT :limit", nativeQuery = true)
    List<Expression> findRandomByUnit(@Param("unit") String unit, @Param("limit") int limit);
